...
int main(int argc, char *argv[]) {
	if (argc < 3 || argc > 4) {
		fprintf(stderr, "Error: The program must be called with 2 or 3 arguments. Usage: \"caesar -e|-d|-a [offset|known_substring] ciphertext\"\n");
		exit(EXIT_FAILURE);
	}
	...
	if ((!strcmp(argv[1], "-d") || !strcmp(argv[1], "-e")) && argc != 4) { // i.e., if ((argv[1] == "-d" || argv[1] == "-e") && argc != 4
